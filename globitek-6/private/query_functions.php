<?php
  ...
  // Find failed login by username
  function find_failed_login($username) {
    global $db;
    $sql = "SELECT * FROM failed_logins ";
    $sql .= "WHERE username='" . db_escape($db, $username) . "';";
    $failed_login = db_query($db, $sql);
    return $failed_login;
  }

  // Insert username to failed_logins
  ...
  // Update failed_login count and last_attempt
  ...
  // Reset failed_login count to zero
