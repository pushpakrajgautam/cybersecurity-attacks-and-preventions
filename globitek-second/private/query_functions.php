<?php
  ...
  function validate_state($state, $errors=array()) {
    ...
    if(!has_valid_name($state['name']))
      $errors[] = "Invalid name";

    if (is_blank($state['code'])) {
      $errors[] = "Code cannot be blank.";
    } elseif (!has_valid_name($state['code'])) {
      $errors[] = "Invalid code";
    }
  ...
  function validate_territory($territory, $errors=array()) {
    ...
    if(!has_valid_name($territory['name']))
      $errors[] = "Invalid name";
  ...
  function validate_salesperson($salesperson, $errors=array()) {
    ...
    if(!phone_number_limit($salesperson['phone'])) {
  ...
  function validate_user($user, $errors=array()) {
    ...
    if(!only_whitelist_characters($user['username']))
      $errors[] = "Username can only use alphabets, numbers and _";
    return $errors;
  }
